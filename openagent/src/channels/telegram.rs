...
/// Build a Telegram channel.
pub fn build(cfg: &TelegramConfig) -> TelegramChannel {
    TelegramChannel::new(cfg.bot_token.clone(), cfg.allowed_users.clone(), cfg.mention_only)
}
...
impl TelegramChannel {
    ...
    async fn persist_allowed_identity(&self, identity: &str) -> anyhow::Result<()> {
        // In OpenAgent, config persistence is handled externally (openagent.yaml / SQLite).
        // Runtime allowlist updates are applied in-memory via add_allowed_identity_runtime.
        let normalized = Self::normalize_identity(identity);
        if normalized.is_empty() {
            anyhow::bail!("Cannot persist empty Telegram identity");
        }
        tracing::info!(
            "Telegram: identity {} approved (runtime only — restart to persist in config)",
            normalized
        );
        Ok(())
    }
