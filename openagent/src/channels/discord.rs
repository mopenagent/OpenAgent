...
/// Build a Discord channel.
pub fn build(cfg: &DiscordConfig) -> DiscordChannel {
    let guild_id = if cfg.guild_id.is_empty() { None } else { Some(cfg.guild_id.clone()) };
    DiscordChannel::new(cfg.token.clone(), guild_id, cfg.allowed_users.clone(), cfg.listen_to_bots, cfg.mention_only)
}
