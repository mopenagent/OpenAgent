//! Mattermost channel — Mattermost API v4.
//!
...
/// Build a Mattermost channel.
pub fn build(cfg: &MattermostConfig) -> MattermostChannel {
    let channel_id = if cfg.channel_id.is_empty() { None } else { Some(cfg.channel_id.clone()) };
    MattermostChannel::new(cfg.url.clone(), cfg.token.clone(), channel_id, cfg.allowed_users.clone(), cfg.thread_replies, cfg.mention_only)
}
