...
class TelegramServiceChannel(McpLiteClient):
    ...
    async def get_status(self) -> dict[str, Any]:
        frame = await self.request({"type": "tool.call", "tool": "telegram.status", "params": {}})
        payload = _decode_tool_result(frame)
        self._status.update(payload)
        return dict(self._status)

    async def get_link_state(self) -> dict[str, Any]:
        frame = await self.request({"type": "tool.call", "tool": "telegram.link_state", "params": {}})
        payload = _decode_tool_result(frame)
        self._status.update(payload)
        return payload
    ...
    def on_event(self, frame: proto.EventFrame) -> None:
        if frame.event == "telegram.connection.status":
            self._status.update(frame.data)
            return
        if frame.event == "telegram.message.received":
