//! OTEL observability — all four pillars — for the Slack service.
//!
//! ```text
//! Pillar   │ Mechanism                              │ Output
//! ─────────┼────────────────────────────────────────┼──────────────────────────────────────
//! Traces   │ tracing::info_span! → OTEL bridge      │ logs/slack-traces-YYYY-MM-DD.jsonl
//! Metrics  │ SlackTelemetry::record()               │ logs/slack-metrics-YYYY-MM-DD.jsonl
...
/// OTEL observability facade for the Slack service.
#[derive(Debug, Clone)]
pub struct SlackTelemetry {
    ...
    pub fn record(&self, data: &Value) {
        ...
        if guard.current_date != today {
            match open_metrics_file(&self.inner.logs_dir, &today) {
                ...
                Err(e) => {
                    eprintln!("slack metrics rotate error: {e}");
                    return;
                }
            }
        }
...
fn open_metrics_file(dir: &PathBuf, date: &str) -> anyhow::Result<File> {
    let path = dir.join(format!("slack-metrics-{date}.jsonl"));
