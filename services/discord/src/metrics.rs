//! OTEL observability — all four pillars — for the Discord service.
//!
//! ```text
//! Pillar   │ Mechanism                              │ Output
//! ─────────┼────────────────────────────────────────┼──────────────────────────────────────
//! Traces   │ tracing::info_span! → OTEL bridge      │ logs/discord-traces-YYYY-MM-DD.jsonl
//! Metrics  │ DiscordTelemetry::record()             │ logs/discord-metrics-YYYY-MM-DD.jsonl
...
/// OTEL observability facade for the Discord service.
#[derive(Debug, Clone)]
pub struct DiscordTelemetry {
    ...
    pub fn record(&self, data: &Value) {
        ...
        if guard.current_date != today {
            match open_metrics_file(&self.inner.logs_dir, &today) {
                ...
                Err(e) => {
                    eprintln!("discord metrics rotate error: {e}");
                    return;
                }
            }
        }
...
fn open_metrics_file(dir: &PathBuf, date: &str) -> anyhow::Result<File> {
    let path = dir.join(format!("discord-metrics-{date}.jsonl"));
...
fn today_date() -> String {
    ...
    let y = 1970 + days / 365;
    let rem = days % 365;
    format!("{y:04}-{:02}-{:02}", (1 + rem / 30).min(12), (1 + rem % 30).min(28))
}
...
pub fn send_ok(channel_id: &str, duration_ms: f64) -> Value {
    json!({
        "ts_ms":       ts_ms(),
        "service":     "discord",
        "op":          "send_message",
...
pub fn send_err(channel_id: &str, duration_ms: f64) -> Value {
    json!({
        "ts_ms":       ts_ms(),
        "service":     "discord",
        "op":          "send_message",
...
pub fn edit_ok(channel_id: &str, message_id: &str, duration_ms: f64) -> Value {
    json!({
        "ts_ms":       ts_ms(),
        "service":     "discord",
        "op":          "edit_message",
...
pub fn edit_err(channel_id: &str, message_id: &str, duration_ms: f64) -> Value {
    json!({
        "ts_ms":       ts_ms(),
        "service":     "discord",
        "op":          "edit_message",
